...
public static class ArcCast {
    ...
    {
        float result = 2f * (reach / gravityOverride.magnitude) * (Mathf.Sin(Vector3.Angle(direction, Vector3.forward) * Mathf.PI / 180f));
        ...
        return Mathf.Max(result * (1 + Mathf.Max(extendedFlightTime, 0f)), 2f); 
    }
}
